...
def crossout(table,row,col):
    ...
    for rowCount in range(tabSize):
            for colCount in range(colSize):
                if colCount == col:
                    table[rowCount].pop(colCount)
                    #rowTab.append(table[rowCount][colCount])
                colCount+=1
    rowCount+=1
...
def place_sums(table):
    ...
    for rowCount in range(tabSize):
        if rowCount == 0:
            ...
            for colCount in range(colSize):
                value = value + table[rowCount][colCount]
                colCount+=1
            table[rowCount].append(value)
            value = 0
        rowCount+=1
